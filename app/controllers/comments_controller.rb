class CommentsController < ApplicationController
  ...
  def update
    @comment = Comment.find(params[:id])
    if @comment.nil?
      redirect_to(root_path, notice: 'Comment not found.') && return
    end
    disallow_non_owner
  ...
  def destroy
    ...
    if @comment.nil?
      redirect_to(root_path,
                  alert: 'Could not find comment to delete.') && return
    end
    unless @comment.owner?(current_user) || @comment.moderatable?(current_user)
      redirect_to(root_path, alert: 'You are not allowed to do that.') && return
    end
    @comment.delete!
    redirect_to build_post_path(@comment.commentable),
